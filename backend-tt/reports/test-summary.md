...
Дата формирования: 2026-05-25T19:28:13.348Z
...
| ---: | --- | --- | --- | ---: |
| 1 | PASS | Регистрация создает пользователя, записывает событие в журнал и возвращает JWT-токены | test/auth-service.test.js | 11.997 |
| 2 | PASS | Регистрация отклоняет занятое имя пользователя | test/auth-service.test.js | 0.495 |
| 3 | PASS | Вход в систему принимает корректные учетные данные и возвращает JWT-токены | test/auth-service.test.js | 4.116 |
| 4 | PASS | Вход в систему отклоняет отсутствующего пользователя и неверный пароль | test/auth-service.test.js | 3.059 |
| 5 | PASS | Смена пароля проверяет старый пароль и обновляет хэш пароля | test/auth-service.test.js | 5.645 |
| 6 | PASS | Смена пароля отклоняет неверный старый пароль | test/auth-service.test.js | 3.566 |
| 7 | PASS | Обновление access token выполняется только по корректному refresh token | test/auth-service.test.js | 0.71 |
| 8 | PASS | Проверка проекта возвращает найденный проект или ошибку 404 | test/project-board-services.test.js | 4.742 |
| 9 | PASS | Стандартные колонки доски создаются только для пустого проекта | test/project-board-services.test.js | 0.799 |
| 10 | PASS | Расчет номера и позиции задачи учитывает максимальные значения | test/project-board-services.test.js | 0.24 |
| 11 | PASS | Сборка доски преобразует колонки, карточки и задачи без колонки в структуру board | test/project-board-services.test.js | 0.509 |
| 12 | PASS | Получение доски проверяет проект, создает стандартные колонки и возвращает board | test/project-board-services.test.js | 0.258 |
| 13 | PASS | Синхронизация доски создает колонки, задачи и записи журнала активности | test/project-board-services.test.js | 1.622 |
| 14 | PASS | Синхронизация доски удаляет отсутствующие во входных данных задачи и колонки | test/project-board-services.test.js | 0.406 |
| 15 | PASS | Нормализация приоритета корректно подготавливает значения для хранения и вывода на доске | test/task-utils.test.js | 2.31 |
| 16 | PASS | Нормализация тегов удаляет пустые значения и ограничивает размер списка | test/task-utils.test.js | 2.506 |
| 17 | PASS | Парсинг параметров планирования принимает корректные значения и отклоняет ошибки | test/task-utils.test.js | 0.686 |
| 18 | PASS | Формирование состояния задачи создает стабильную структуру для сравнения | test/task-utils.test.js | 0.213 |
| 19 | PASS | Парсинг идентификаторов принимает только положительные числовые значения | test/task-utils.test.js | 0.101 |
